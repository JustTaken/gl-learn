...
fn vertex_specification(gl: *OpenGL) !void {
    ...
    const index_buffer = [_]u32 {0, 2, 1, 3, 2, 0, 0, 1, 4, 4, 1, 5, 5, 1, 6, 6, 2, 1, 6, 5, 4, 4, 7, 6, 7, 4, 0, 3, 0, 7, 3, 7, 2, 2, 7, 6};
    ...
    c.glDisableVertexAttribArray(position_location);
...
fn pre_draw(gl: *OpenGL) void {
    ...
    const projection: [4][4]f32 = math.Matrix.perspective(10.0 * std.math.pi / 180.0, @as(f32, SCREEN_WIDTH) / @as(f32, SCREEN_HEIGHT), 0.1, 100.0);
